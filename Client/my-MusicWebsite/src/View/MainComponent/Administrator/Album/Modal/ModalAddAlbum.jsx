...
const ModalAddAlbum = ({ openModal, handleClose ,setLoading }) => {
  ModalAddAlbum.propTypes = {
     ...
     const resetForm = () => {
      ...
      const handelSave = async () => {
        ...
        formData.append("TenAlbum", obj?.tenChude);
        formData.append("NgayPhatHanh", selectedDate);
        formData.append("Url", imageDataBasic);
            const response = await axios.post("https://localhost:7280/api/Album/addalbum", formData, config);
  ...
  >
    <DialogTitle>{"Thêm mới album"}</DialogTitle>
    <DialogContent >
      <DialogContentText id="alert-dialog-slide-description">
        <Grid2 container sx={{width:"500px"}} spacing={2} >
            <Grid2 sx={{width:"500px"}}>
              <TextField id="standard-basic" onChange={handleChange("tenAlbum")} label="Tên album" variant="standard" fullWidth />
            </Grid2>
